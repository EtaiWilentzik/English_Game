...
def class_rate():
    try:
        ...
        cursor = cnx.cursor(buffered=True)
        query = f"""
             SELECT COUNT(*) as position,(SELECT COUNT(*) FROM user) AS total_users
            FROM 
            (SELECT u.username, (SELECT COUNT(*) FROM userlog WHERE userlog.username = u.username AND userlog.result = 'Win') AS winCount 
                     FROM user as u ) as temp 
            WHERE winCount >= (SELECT COUNT(*) FROM userlog WHERE username = '{username}'
            AND result="Win" ) 
            """
